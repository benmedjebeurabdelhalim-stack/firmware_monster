...
// ==========================================
// VOLTSHIELD X: SOVEREIGN DISPLAY INJECTION
// ==========================================
#include "MyDisplay.h" 
MyCustomDisplay my_sovereign_display; 
// ==========================================
...
void __attribute__((weak)) taskInputHandler(void *parameter) {
    ...
    while (true) {
        ...
        if (!AnyKeyPress || millis() - timer > 75) {
            ...
            // ==========================================
            // VOLTSHIELD X: VIRTUAL BUTTON ENGINE (TOUCH)
            // ==========================================
            int32_t touch_x, touch_y;
            if (my_sovereign_display.getTouch(&touch_x, &touch_y)) {
                AnyKeyPress = true;
                // تقسيم الشاشة 320x480 للتحكم بالواجهة
                if (touch_y < 160) {
                    UpPress = true;
                    PrevPress = true; // يحاكي زر الصعود
                } else if (touch_y > 320) {
                    DownPress = true;
                    NextPress = true; // يحاكي زر النزول
                } else {
                    if (touch_x > 160) {
                        SelPress = true; // الجانب الأيمن للاختيار (Enter)
                    } else {
                        EscPress = true; // الجانب الأيسر للرجوع (Back)
                    }
                }
            }
            // ==========================================
...
void setup_gpio() {
    ...
    // ==========================================
    // VOLTSHIELD X: HARDWARE CONTROL LAYER
    // ==========================================
    pinMode(16, OUTPUT);
    digitalWrite(16, HIGH); // تشغيل إجباري لموديم 4G (A7670G)
    
    Serial1.begin(115200, SERIAL_8N1, 18, 17); // تهيئة ناقل الاتصال الخاص بالموديم (RX=18, TX=17)
    
    pinMode(1, INPUT); // تهيئة دبوس الرادار (RCWL-0516)
    // ==========================================
...
void setup() {
    ...
    // ==========================================
    // VOLTSHIELD X: INITIALIZE SOVEREIGN HARDWARE FIRST
    // ==========================================
    my_sovereign_display.begin(); 
    // ==========================================
